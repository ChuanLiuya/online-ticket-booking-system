...
const { searchEvents } = useEventActions()
const route = useRoute()

const currentPage = ref(1)
const pageSize = ref(10)
const searchKeyword = ref('')
const searchEventsResult = ref<Event[]>([])
const total = ref(0)

const searchEventsCut = computed(() => {
  const titleMaxLength = 20
  const descriptionMaxLength = 100
  const locationMaxLength = 20
  return searchEventsResult.value.map((event) => {
...
const handleSearch = async () => {
  currentPage.value = 1
  try {
    const res = await searchEvents(searchKeyword.value, { page: currentPage.value, limit: pageSize.value })
    searchEventsResult.value = res.events || []
...
<template>
  <div class="container" id="container">
    <div class="title">搜索活动</div>
    <div class="search-bar">
      <el-input
        placeholder="输入活动名称、描述或地点进行搜索"
    ...
    <div class="event-list">
      <div
        v-for="event in searchEventsCut"
        :key="event.id"
        class="event-item"
    ...
    <el-empty class="no-events" v-if="searchEventsCut.length === 0" description="暂无匹配的活动" />
